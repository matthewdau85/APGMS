...
export async function buildEvidenceBundle(abn: string, taxType: string, periodId: string) {
  ...
  const deltas = ledgerRows.rows;
  const last = deltas[deltas.length - 1];
  const bundle = {
    bas_labels: { W1: null, W2: null, "1A": null, "1B": null },
    rpt_payload: rptRow.rows[0]?.payload ?? null,
    rpt_signature: rptRow.rows[0]?.signature ?? null,
    owa_ledger_deltas: deltas,
    bank_receipt_hash: last?.bank_receipt_hash ?? null,
    anomaly_thresholds: periodRow.rows[0]?.thresholds ?? {},
    discrepancy_log: [] as unknown[],
  };
  return bundle;
}
