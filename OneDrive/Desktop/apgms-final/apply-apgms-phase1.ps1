# --- begin file: apply-apgms-phase1-fixed.ps1
param(
...
# ---------- Overwrite Makefile with Windows-friendly content ----------
W "Makefile" @'
.PHONY: bootstrap dev test lint typecheck compose-up seed e2e osf\\:evidence

bootstrap:
	@echo [bootstrap] Python venv + Poetry + deps
	@powershell -NoProfile -Command "if (!(Test-Path .venv)) { py -3.12 -m venv .venv }"
...
# ---------- Ensure libs/py-sdk with deps inc. cbor2 ----------
W "libs/py-sdk/pyproject.toml" @'
[tool.poetry]
name = "apgms-py-sdk"
version = "0.1.0"
...
W "libs/py-sdk/apgms_sdk/rpt.py" @'
...
def issue_rpt(payload: dict, sk_bytes: bytes) -> str:
    ...
    # Deterministic payload: list of tuples for stable order
...
def introspect(token: str) -> dict:
    ...
    return d
'@
W "libs/py-sdk/tests/test_rpt.py" @'
...
# ---------- P01 JSON Schemas + Avro stub + codegen ----------
W "libs/schemas/json/payroll_event.v1.json" @'{ "$schema":"https://json-schema.org/draft/2020-12/schema","title":"PayrollEventV1","type":"object","required":["version","event_type","monotonic_seq","dedupe_key","src_hash","signed_at","producer_id","employee_tax_file_number","gross_cents","pay_period_start","pay_period_end","withholding_cents"],"properties":{"version":{"const":"v1"},"event_type":{"const":"payroll"},"monotonic_seq":{"type":"integer","minimum":0},"dedupe_key":{"type":"string","minLength":8},"src_hash":{"type":"string","pattern":"^[A-Fa-f0-9]{64}$"},"signed_at":{"type":"string","format":"date-time"},"producer_id":{"type":"string"},"employee_tax_file_number":{"type":"string"},"employee_id":{"type":["string","null"]},"pay_period_start":{"type":"string","format":"date"},"pay_period_end":{"type":"string","format":"date"},"gross_cents":{"type":"integer","minimum":0},"withholding_cents":{"type":"integer","minimum":0},"super_cents":{"type":["integer","null"],"minimum":0},"notes":{"type":["string","null"]}} }'
W "libs/schemas/json/pos_event.v1.json" @'{ "$schema":"https://json-schema.org/draft/2020-12/schema","title":"POSEventV1","type":"object","required":["version","event_type","monotonic_seq","dedupe_key","src_hash","signed_at","producer_id","txn_id","lines"],"properties":{"version":{"const":"v1"},"event_type":{"const":"pos"},"monotonic_seq":{"type":"integer","minimum":0},"dedupe_key":{"type":"string"},"src_hash":{"type":"string","pattern":"^[A-Fa-f0-9]{64}$"},"signed_at":{"type":"string","format":"date-time"},"producer_id":{"type":"string"},"txn_id":{"type":"string"},"lines":{"type":"array","items":{"type":"object","required":["sku","qty","unit_price_cents","tax_code"],"properties":{"sku":{"type":"string"},"qty":{"type":"number"},"unit_price_cents":{"type":"integer"},"discount_cents":{"type":["integer","null"],"minimum":0},"tax_code":{"type":"string","enum":["GST","GST_FREE","INPUT_TAXED"]},"return_for_txn":{"type":["string","null"]}}}}}}'
W "libs/schemas/json/bank_event.v1.json" @'{ "$schema":"https://json-schema.org/draft/2020-12/schema","title":"BankEventV1","type":"object","required":["version","event_type","monotonic_seq","dedupe_key","src_hash","signed_at","producer_id","direction","amount_cents","reference"],"properties":{"version":{"const":"v1"},"event_type":{"const":"bank"},"monotonic_seq":{"type":"integer"},"dedupe_key":{"type":"string"},"src_hash":{"type":"string","pattern":"^[A-Fa-f0-9]{64}$"},"signed_at":{"type":"string","format":"date-time"},"producer_id":{"type":"string"},"direction":{"type":"string","enum":["CREDIT","DEBIT"]},"amount_cents":{"type":"integer"},"reference":{"type":"string"},"rail_id":{"type":["string","null"]},"bank_receipt_hash":{"type":["string","null"]}} }'
W "libs/schemas/json/recon_transition.v1.json" @'{ "$schema":"https://json-schema.org/draft/2020-12/schema","title":"ReconTransitionV1","type":"object","required":["version","period_id","tax_type","from","to","at","reason"],"properties":{"version":{"const":"v1"},"period_id":{"type":"string","pattern":"^\\d{4}-\\d{2}$"},"tax_type":{"type":"string","enum":["GST","PAYGW"]},"from":{"type":"string"},"to":{"type":"string"},"at":{"type":"string","format":"date-time"},"reason":{"type":"string"}} }'
W "libs/schemas/json/error_envelope.v1.json" @'{ "$schema":"https://json-schema.org/draft/2020-12/schema","title":"ErrorEnvelopeV1","type":"object","required":["version","code","message","occurred_at","dedupe_key"],"properties":{"version":{"const":"v1"},"code":{"type":"string"},"message":{"type":"string"},"occurred_at":{"type":"string","format":"date-time"},"dedupe_key":{"type":"string"},"context":{"type":"object"}} }'

W "libs/protocols/avro/payroll_event.v1.avsc" @'
{ "type":"record","name":"PayrollEventV1","namespace":"apgms.avro",
  "fields":[
    {"name":"version","type":{"type":"enum","name":"Version","symbols":["v1"]}},
    {"name":"event_type","type":{"type":"enum","name":"EvtType","symbols":["payroll"]}},
    {"name":"monotonic_seq","type":"long"},
    {"name":"dedupe_key","type":"string"},
    {"name":"src_hash","type":"string"},
    {"name":"signed_at","type":"string"},
    {"name":"producer_id","type":"string"},
    {"name":"employee_tax_file_number","type":"string"},
    {"name":"employee_id","type":["null","string"],"default":null},
    {"name":"pay_period_start","type":"string"},
    {"name":"pay_period_end","type":"string"},
    {"name":"gross_cents","type":"long"},
    {"name":"withholding_cents","type":"long"},
    {"name":"super_cents","type":["null","long"],"default":null},
    {"name":"notes","type":["null","string"],"default":null}
  ] }
'

W "libs/schemas/codegen/generate.py" @'
import json, pathlib
...
def to_ts(name, props, required):
    ...
    for k,v in props.items():
        ...
        elif v.get("type") == "object": t="Record<string, any>"
...
W "apps/services/tax-engine/app/tax_rules.py" @'
...
'
W "apps/services/tax-engine/tests/test_tax_rules.py" @'
...
W "libs/schemas/tests/test_evolution.py" @'
...
Write-Host "`n[Phase 1] Files written. Next:"
Write-Host "  1) cd $RepoRoot"
Write-Host "  2) Copy-Item .env.example .env -Force"
Write-Host "  3) py -3.12 -m venv .venv; .\\.venv\\Scripts\\Activate.ps1"
Write-Host "  4) pip install -U pip; pip install poetry"
Write-Host "  5) make bootstrap"
Write-Host "  6) make test"
Write-Host "  7) make compose-up"
# --- end file
