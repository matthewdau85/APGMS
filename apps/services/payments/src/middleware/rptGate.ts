...
export async function rptGate(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const q = `
      SELECT id as rpt_id, payload_c14n, payload_sha256, signature, expires_at, status, nonce, kms_key_id
      FROM rpt_tokens
    ...
    (req as any).rpt = { rpt_id: r.rpt_id, nonce: r.nonce, payload_sha256: r.payload_sha256, kid: r.kms_key_id };
    return next();
  } catch (e: any) {
