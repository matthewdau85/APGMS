...
export async function deposit(req: Request, res: Response) {
  try {
    ...
    try {
      ...
      const prevBal = last[0]?.balance_after_cents ? Number(last[0].balance_after_cents) : 0;
      const prevHash = last[0]?.hash_after ? String(last[0].hash_after) : "";
      const newBal = prevBal + amt;

      const transferUuid = randomUUID();
      const receiptHash = sha256Hex(`synthetic:${transferUuid}`);
      ...
      const { rows: ins } = await client.query(
        ...
        [abn, taxType, periodId, transferUuid, amt, newBal, receiptHash, prevHash, hashAfter]
      );
