# apply_server_update.ps1
$ErrorActionPreference = 'Stop'

# 0) Paths
$ServerPath = ".\server.js"
$BackupPath = ".\server.js.bak.$(Get-Date -Format yyyyMMdd_HHmmss)"

# 1) Backup current server.js (if present)
if (Test-Path $ServerPath) {
  Copy-Item $ServerPath $BackupPath -Force
  Write-Host "Backed up server.js -> $BackupPath"
}

# 2) Write the new server.js (UTF-8 without BOM)
$ServerJS = @'
...
// (C) Release (simulated debit) – maintains hash chain & state
...
[System.IO.File]::WriteAllText((Resolve-Path $ServerPath), $ServerJS, $utf8NoBom)
Write-Host "Wrote new server.js ✅"

# 3) Make sure deps exist (idempotent)
npm i express body-parser pg tweetnacl dotenv | Out-Host

# 4) (Optional) re-apply migration 002 (idempotent)
$env:PGPASSWORD = 'apgms_pw'
if (Test-Path .\migrations\002_patent_extensions.sql) {
  psql -h 127.0.0.1 -U apgms -d apgms -f .\migrations\002_patent_extensions.sql | Out-Host
}
