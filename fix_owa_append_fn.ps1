...
# Try to pull defaults from .env.local if present
$envFile = ".\.env.local"
...
$sql = @"
CREATE OR REPLACE FUNCTION public.owa_append(
  p_abn           text,
  p_tax           text,
  p_period        text,
  p_amount        bigint,
...
DECLARE
  ...
  v_hash_after    text;
BEGIN
  -- Get the latest balance/hash for this (abn,tax,period)
  ...
  -- Simple deterministic chain hash using built-in md5
...
if ($LASTEXITCODE -ne 0) {
  throw "Failed to replace owa_append function."
} else {
  Write-Host "owa_append function replaced successfully. ✅"
}
