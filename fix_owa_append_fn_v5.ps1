...
# Pull creds from .env.local if present
$envFile = ".\.env.local"
...
# Note: single-quoted PS string so $func$ stays literal
$sql = @'
BEGIN;
...
DECLARE
  ...
  v_prev_hash     text;
  v_hash_after    text;
  v_transfer_uuid uuid   := (md5(random()::text || clock_timestamp()::text))::uuid;
BEGIN
